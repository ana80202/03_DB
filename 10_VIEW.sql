 ...
 * 
 * ****VIEW 사용 시 주의사항****
 * 1) 가상 테이블(실제 테이블X) --> ALTER구문 사용 불가
 * 2) VIEW를 이용한 DML(INSERT/UPDATE/DELETE)이
 *    가능한 경우도 있지만
 *    많은 제약이 따르기 때문에 SELECT용도로 사용 하는 것을 권장
 *
 *
 *[VIEW생성방법]
 ...
 *[WITH READ ONLY];
 *
 *--1) OR REPALCE : 기존에 동일한 뷰이름이 존재하는 경우 덮어쓰고,
 *                  존재하지 않으면 새로 생성.
 * 
 * --2) FORCE | NOFORCE
 ...
 *
 *--4) WITH READ ONLY : 뷰에 대해 조회만 가능(DML 수행 불가) --읽기 전용 
 *
 * */

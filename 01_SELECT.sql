...
--SELECT : 조회  * :모든 FROM : 어디에서

/*SELECT (DQL 또는 DML) : 조회
 *
 * -데이터를 조회(SELECT) 하면 조건에 맞는 행들이 조회됨.
 * 이 떄, 조회된 행들의 집합을 "RESULT SET" (조회 결과의 집합) 이라고 함.
 *
...
SELECT EMP_ID,EMP_NAME,SALARY,SALARY*12 FROM EMPLOYEE; --;을 기준으로 SQL 구문이 나뉨
...
--DUAL(DUmmy tAbLe) 테이블 : 가짜 테이블(임시 조회용 테이믈)
...
/*
 * 컬럼명 AS 별칭 : 별칭쓸때 띄어쓰기 x EX) 현재시간(O) 현재 시간(X),특수문자X ,문자만 O
 * 
 * 컬럼명 AS "별칭" : 별칭쓸때 띄어쓰기 가능, 특수문자 가능, 문자만 쓰는 것도 가능
 * 
 * AS는 생략 가능 
 * 
 * */
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; --비교연산자(=) 같다라는 의미 / 대입연산자(:=)
...
-- EMPOYEE 테이블에서 급여가 300만 미만 또는 500만 이상인 사원의
-- 사번, 이름 , 급여, 전화번호 조회
SELECT EMP_ID,EMP_NAME,SALARY,PHONE
FROM EMPLOYEE
WHERE SALARY < 3000000 OR SALARY >= 50000000;

--EMPOYEE 테이블에서 급여가 300만 이상500만 미만인 사원의
-- 사번, 이름 , 급여, 전화번호 조회
SELECT EMP_ID,EMP_NAME,SALARY,PHONE
FROM EMPLOYEE
WHERE SALARY >= 3000000 AND SALARY < 50000000;
...
--NOT BETEEN A AND B : A 이상 B 이하가 아닌 경우 --> A 미만, B초과
SELECT EMP_ID,EMP_NAME,SALARY
FROM EMPLOYEE
...
--WHERE EMP_NAME LIKE '전__'; 이거는 이름이 세글자인 사람만 나옴
...
--WHERE PHONE LIKE '010________'; 이거는 전화번호가 8글자인것만 나온다
...
--LIKE의 ESCAPE OPTION: 일반 문자로 처리할 '_'/'%'앞에 아무 특수기호나 첨부해서 구분하게함
SELECT EMP_NAME,EMAIL
FROM EMPLOYEE

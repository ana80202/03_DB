...
--함수는 SELECT문의
--SELECT절, WHERE 절, ORDER BY절, GROUO BY절, HAVING절 사용가능
...
-- 문자열을 앞에서부터 검색하여 첫 번쨰 B위치 조회
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; --3

--문자열을 5번째 문자부터 검색하여 첫 번쨰 B 위치를 조회
SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL; --9

--문자열을 5번쨰 문자부터 검색하여 두 번째 B위치를 조회
...
SELECT EMP_ID,EMP_NAME
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2) != 0;   --WHERE MOD(EMP_ID, 2) <> 0; <> : 같지 않다
...
SELECT TRUNC(123.456, -2) FROM DUAL; --100의 자리 아래 버림
 
/*버림, 내림 차이점*/

SELECT FLOOR(-123.5),TRUNC(-123.5) FROM DUAL;  
--    -124(FLOOR입장에서는 124와 가깝기떄문에)     TRUNC-123(정수 부분까지만 나옴)
...
-- 2024-02-29 11:20:33.770 +0900(UTC에서 +9시간 나타냄, 한국 표준시)

--MONTHS_BETWEEN(날짜, 날짜)
SELECT  ROUND (MONTHS_BETWEEN(SYSDATE, '2024-06-26'),3) "수강기간(개월)"
FROM DUAL;  --MONTHS BETWEEN은 보통 ROUND와 묶어서 많이 사용

--EMPLOYEE테이블에서
--사원의 이름, 입사일 , 근무한 개월 수, 근무년차 조회
SELECT EMP_NAME, HIRE_DATE,
CEIL (MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) ||'개월' "근무한 개월 수",
CEIL (MONTHS_BETWEEN(SYSDATE, HIRE_DATE)/ 12) || '년차' "근무 년차"   -- || : 연결 연산자
FROM EMPLOYEE;
...
SELECT EMP_NAME 사원명,
       ...
       EXTRACT(DAY FROM HIRE_DATE) || '일' AS 입사일
			 FROM EMPLOYEE
			 WHERE	HIRE_DATE >'2000-01-01' --입사일 2000년 1월 1일 이후 부터
			 ORDER BY HIRE_DATE;   --입사일 순서대로 나옴
...
SELECT TO_CHAR(1234) FROM DUAL; --1,234 -> '1234' 문자형으로 변환해줌
...
SELECT TO_CHAR(SYSDATE, 'YY/MM/DD HH24:MI:SS DAY') FROM DUAL; 
...
-- TO_DATE(숫자형데이터, [포맷]) : 숫자형 데이터를 날짜로 변경
--> 지정된 포맷으로 날짜를 인식함
...
--EMPLOYEE테이블에서 보너스르 받으면'0' 안받으면 'X' 조회
...
SELECT MIN(SALARY) 급여, MIN(HIRE_DATE) 입사일, MIN(EMAIL) 이메일
FROM EMPLOYEE; 

--급여 최대값, 가장 늦은 입사일, 알파벳순서가 가장 느린 이메일을 조회
SELECT MAX(SALARY) 급여, MAX(HIRE_DATE) 입사일, MAX(EMAIL) 이메일
FROM EMPLOYEE; 

--EMPLOYEE 테이블에서 급여를 가장 많이 받는 사원의
-- 이름, 급여 , 직급코드를 조회
SELECT EMP_NAME 사원명,SALARY 급여,JOB_CODE 직급코드
FROM EMPLOYEE

 ...
 *  --> CHAR(10) (무조건 10바이트짜리 저장공간을 만듦) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간 모두 사용
 * 
 * VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE)
 *   --> VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE는 반환한다.
 * 
 * DATE : 날짜타입
 * BLOB : 대용량 이진 데이터(4GB)
 * CLOB : 대용량 문자 데이터(4GB)
 ...
 * 데이터 무결성 보장을 목적으로 함. --> 데이터가 정확하고 일관된 상태(중복데이터가 없는상태)
 * 
 * +입력 데이터에 문제가 없는지 자동으로 검사하는 목적
 * +데이터의 수정/ 삭제 기능 여부 검사등을 목적으로 함
...
--NULL을 ("KH_CDY"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
--* 제약조건 지정 방법
--1) 컬럼 레벨 : [CONSTRAINT 제약조건명] 제약조건                        --대괄호는 생략 가능
--2) 테이블 레벨 : [CONSTRAINT 제약조건명] 제약조건(컬럼명)

--UNIQUE 제약 조건 테이블 생성
CREATE TABLE USER_USED_UK(  --UNIQUE 줄여서 UK 라고 많이 부름
		  USER_NO NUMBER,
...
INSERT INTO USER_USED_UK
VALUES(1,'USER01','PASS01','홍길동','남','010-1234-5678','hong123@kh.or.kr');
--ORA-00001: 무결성 제약 조건(KH_CDY.USER_ID_U)에 위배됩니다
...
--ORA-00001: 무결성 제약 조건(KH_CDY.USER_ID_NAME_U)에 위배됩니다
...
--한 개 컬럼에 설정할 수 있고, 여러개 컬럼을 묶어서 설정할 수 있음
...
INSERT INTO USER_USED_PK
VALUES(1,'USER02','PASS02','이순신','남','010-5679-5678','lee123@kh.or.kr');
--기본키 중복 오류
--ORA-00001: 무결성 제약 조건(KH_CDY.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL,'USER03','PASS03','유관순','여','010-9999-5678','yoo123@kh.or.kr');
--기본키 NULL 이므로 오류
--ORA-01400: NULL을 ("KH_CDY"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
INSERT INTO USER_USED_PK2
VALUES(2,'USER01','PASS02','유관순','남','010-1234-5678','hong123@kh.or.kr');

INSERT INTO USER_USED_PK2
VALUES(1,'USER01','PASS02','신사임당','여','010-1234-5678','hong123@kh.or.kr');
--ORA-00001: 무결성 제약 조건(KH_CDY.PK_USERNO_USERID)에 위배됩니다
--회원번호와 아이디 둘 다 중복되었을때만 제약조건 위배 에러 발생

INSERT INTO USER_USED_PK2
VALUES(NULL,'USER01','PASS02','신사임당','여','010-1234-5678','hong123@kh.or.kr');
--ORA-01400: NULL을 ("KH_CDY"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- 테이블레벨일 경우
-- [CONSTRAINT 이름] FOREIGN KEY (적용할 컬럼명) REFERENCES 참조할 테이블명 [(참조할컬럼)] [삭제롤]
...
INSERT INTO USER_USED_FK
VALUES(2,'USER02','PASS02','이순신','남','010-5678-9078','lee123@kh.or.kr',10);

INSERT INTO USER_USED_FK
VALUES(3,'USER03','PASS03','유관순','여','010-9904-5628','yoo123@kh.or.kr',30);

INSERT INTO USER_USED_FK
VALUES(4,'USER04','PASS04','안중근','남','010-2314-1218','ahn@kh.or.kr',NULL);
--> NULL 사용 가능

INSERT INTO USER_USED_FK
VALUES(5,'USER05','PASS05','윤봉길','남','010-666-1234','yoon@kh.or.kr',50);
--ORA-02291: 무결성 제약조건(KH_CDY.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
--부모 키가 없습니다 -> 50이라는게 부모한테 없다(참조할 수 있는 값이 없다)
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE컬럼에서 제공하는 값이 아니므로
...
--ORA-02292: 무결성 제약조건(KH_CDY.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
--- 자식 레코드가 발견되었습니다 --> 자식이 이미 사용하고 있어 사용하지 못 한다라는 뜻
...
SELECT * FROM USER_GRADE2;
SELECT * FROM USER_USED_FK2; --10을 참조하고 있던 GRADE_CODE가 NULL로 바뀜
--10을 가졌던 자식테이블 값이 NULL이 됨을 확인.
...
--ON DELETE CACADE 옵션으로 인해 참조키를 사용한 행이 삭제됨을 확인
SELECT * FROM USER_GRADE3;
SELECT * FROM USER_USED_FK3; --아예 행이 없어짐 
...
--ORA-02290: 체크 제약조건(KH_CDY.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	USER_NO NUMBER CONSTRAINT PK_USER_TEST PRIMARY KEY, --회원번호 --VARCHAR도 괜찮음
	USER_ID VARCHAR2(20) CONSTRAINT UK_USER_ID UNIQUE, -- 회원아이디
	USER_PWD VARCHAR2(20) CONSTRAINT NN_USER_PWD NOT NULL, --회원비밀번호
	PNO VARCHAR2(20) CONSTRAINT NN_PNO NOT NULL , -- 주민등록번호
	GENDER VARCHAR2 (3) CONSTRAINT CK_GENDER CHECK(GENDER IN ('남','여'))   , --성별
	PHONE VARCHAR2 (30), --연락처
	ADDRESS VARCHAR2 (100), --주소
	STATUS VARCHAR2(3) DEFAULT 'N' CONSTRAINT NN_STATUS NOT NULL,   	--탈퇴여부
	CONSTRAINT CK_STATUS CHECK(STATUS IN ('Y','N')),
	CONSTRAINT UK_PNO UNIQUE(PNO)  --중복 허용 안함
);
...
INSERT INTO USER_TEST
VALUES(2,'user02','pass02','880222-1234567','여','010-2221-9999','서울시 강남구 역삼동','N');
INSERT INTO USER_TEST
VALUES(3,'user03','pass03','900322-1234567','여','010-3211-9999','서울시 강남구 역삼동','Y');
INSERT INTO USER_TEST
VALUES(4,'user04','pass04','910122-1234567','남','010-2511-9999','서울시 강남구 역삼동','N');
INSERT INTO USER_TEST
VALUES(5,'user05','pass05','930122-1224567','여','010-2511-9999','서울시 강남구 역삼동','Y');
...
-- 제약조건, 코멘트는 복사 되지 않기 떄문에 별도 추가 작업이 필요하다!
...
--> 참조 테이블의 PK(PRIMARY KEY =기본키)를 FK(FOREIGN KEY = 외래키)로 사용하는 경우 참조컬럼명 생략 가능
...
FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT ON DELETE SET NULL; -- ON DELETE SET NULL : 부모키가 삭제되면 자식키는 NULL값으로 변하는 삭제옵션
                                 /*PK 컬럼 참조 (DEPT_ID)*/     
...
--DEPARTMENT 테이블의 LOCATION_ID에 외래키 제역조건 추가

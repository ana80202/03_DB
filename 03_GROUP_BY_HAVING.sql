 ...
 * 6 : ORDER BY 컬럼명 | 별칭 | 컬럼순번 정렬방식(ASC/DESC) [NULLS FIRST | LAST]; ->정렬은 제일 마지막
 * 
 * */
...
--그룹함수는 단 한개의 결과값만 산출하기 떄문에 그룹이 여러개일 경우 오류 발생
...
FROM EMPLOYEE;
--ORA-00937: 단일 그룹의 그룹 함수가 아닙니다 에러 발생!
...
SELECT JOB_CODE 직급코드,ROUND(AVG(SALARY)) 급여평균,COUNT(*) 인원수
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE; 
...
/*3*/SELECT DECODE( SUBSTR(EMP_NO,8,1),'1','남','2','여') 성별, --8번쨰 자리에서 1글자 읽었을때 1이면 남 2이면 여
			 COUNT(*) "인원 수", 
			 SUM(SALARY) "급여 합"
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DECODE( SUBSTR(EMP_NO,8,1),'1','남','2','여') --별칭 사용 불가능!
/*4*/ORDER BY "인원 수"; --별칭 사용 가능(SELECT절 해석 완료 했기 떄문)
...
--부서코드 오름차순(기본값), 직급코드 내림차순

 SELECT DEPT_CODE, JOB_CODE, COUNT(*)
 FROM EMPLOYEE
 GROUP BY DEPT_CODE;
--ORA-00979: GROUP BY 표현식이 아닙니다. 에러발생!
...
FROM EMPLOYEE
--WHERE SALARY >= 3000000 --> 한 사람의 급여가 3백만원 이상이라는 조건(요구사항 충족 x)
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE그룹 중 급여 평균이 3백만 이상인 그룹만 조회
ORDER BY DEPT_CODE;
...
--EMPLOYEE 테이블에서 부서코드가 'D5'인  사번, 이름, 부서코드, 급여
SELECT EMP_ID,EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
WHERE DEPT_CODE ='D5'
--UNION  --두개의 SELECT문을 합침
--INTERSECT
--UNION ALL --합집합에서 중복제거 안하고 나옴
MINUS --A(앞)와 B(뒤)에서 제거 하고 남은 것 (차집합)

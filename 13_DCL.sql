...
--GRANT RESOURCE , CONNECT TO kh_cdy;
...
CREATE USER sample1234 IDENTIFIED BY sample1234 ;


--2. 새 연결 추가
--> ORA -01045 : 사용자 cdy_sample는 CREATE SESSION 권한을 가지고 있지 않음 / 로그온이 거절되었습니다
--> 접속 권한 (CREATE SESSION) 없어서 오류 발생

--3. 접속 권한 
--[권한 부여]
--GRANT CREATE SESSION , 권한, 권한 ... TO 사용자 명;
GRANT CREATE SESSION TO cdy_sample;
...
GRANT CREATE TABLE TO cdy_sample;

ALTER USER cdy_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO cdy_sample;
...
--*객체 권한*
--kh_cdy / cdy_sample 사용자 계정끼리 서로 객체 접근 권한 부여

--1. (sample) kh_cdy 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_cdy.EMPLOYEE;
...
GRANT SELECT ON EMPLOYEE TO cdy_sample;
...
REVOKE SELECT ON EMPLOYEE FROM cdy_sample;

--5.(SAMPLE) 권한 회수 확인
SELECT * FROM kh_cdy.EMPLOYEE;
--ORA-00942: 테이블 또는 뷰가 존재하지 않습니다

--SYS(최고 관리자 계정) --> 사용자를 만들고 권한을 붙여줌 **이거 꼭 기억하기**

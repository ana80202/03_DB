...
--1) 노옹철의 부서코드를 조회(서브쿼리)
SELECT DEPT_CODE
FROM EMPLOYEE
WHERE EMP_NAME = '노옹철'; --D9

--2) 부서코드가 D9인 직원의 이름, 부서코드 조회하기(메인쿼리)
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';

--3) 부서코드가 노옹철 사원과 같은 소속의 직원 명단 조회
--> 위의 2개의 단계를 하나의 쿼리로 만들기 --> 1번의 쿼리문을 서브쿼리로
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE = (SELECT DEPT_CODE
									 FROM EMPLOYEE
									 WHERE EMP_NAME = '노옹철');  --서브쿼리 작성 시 괄호 안에 ; 있으면 안됨!

--서브쿼리 예시 2.
-- 전 직원의 평균 급여(서브쿼리)보다 많은 급여를 받고 있는 직원(WHERE 절)의
--사번, 이름, 직급코드, 급여 조회(메인쿼리)
--문제 잘 읽어보고 메인쿼리가 뭔지, 서브쿼리가 되어야 하는게 뭔지 파악하기							 
 ...
 * 
 * -다중행 (단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 여러개 일 때 (IN)
 * 
 * -다중열 서브쿼리 : 서브쿼리의 SELECT 절에 나열된 항목수가 여러개일 때(열은 여러개 인데 행은 1개)
 * 
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID) -- (메인쿼리)  --연결고리가 될 컬럼명이 다르므로 ON을 써줌
...
FROM EMPLOYEE
NATURAL JOIN JOB --동일한 컬럼, 값이 있기떄문에 NATURAL JOIN 사용
...
--3) 위의 두 쿼리를 합쳐 부서별 급여합이 큰 부서의 부서명, 급여 합 조회
SELECT DEPT_TITLE,SUM(SALARY)
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
GROUP BY DEPT_TITLE
...
--1) 사수에 해당하는 사원번호 조회
SELECT DISTINCT  MANAGER_ID  --DISTINCT 중복값 제거
FROM EMPLOYEE
...
WHERE EMP_ID NOT IN (SELECT DISTINCT  MANAGER_ID
                 FROM EMPLOYEE
                 WHERE MANAGER_ID IS NOT NULL);  -- 항상 마지막에 ;찍어주기 중간에 찍으면 X
...
FROM EMPLOYEE
NATURAL JOIN JOB 
WHERE JOB_NAME = '과장';  -- 최소 급여 : 2200000

--3) 대리 직급의 직원들 중에서 과장 직급의 최소 급여보다 많이 받는 직원
--3-1) MIN을 이용해서 단일행 서브쿼리로 만들어 구하는 방법
SELECT EMP_ID,EMP_NAME,JOB_NAME,SALARY
FROM EMPLOYEE
NATURAL JOIN JOB 
WHERE JOB_NAME = '대리'
...
AND SALARY > ANY(SELECT SALARY
                 FROM EMPLOYEE
                 NATURAL JOIN JOB 
                 WHERE JOB_NAME = '과장'); -- 최소 급여 : 2200000보다 더 크냐
...
--DEPARTMENT 테이블의 LOCTION_ID와 동일한 DEPT_ID가
...
--2) DEPARTMENT 테이블에서 위의 결과와 동일한 LOCTION_ID를 가지고 있는 DEPT_ID 조회
SELECT DEPT_ID
FROM DEPARTMENT
...
-- 사번,이름,부서코드,직급코드,입사일
SELECT HIRE_DATE,DEPT_CODE,JOB_CODE
FROM EMPLOYEE
WHERE HIRE_DATE ;
...
WHERE (DEPT_CODE,MANAGER_ID) = (SELECT DEPT_CODE,MANAGER_ID
                                FROM EMPLOYEE
                                WHERE SUBSTR(EMP_NO,1,2)= '77' -- 1번째 자리에서 2번째 까지 자른 값
                                AND SUBSTR(EMP_NO,8,1) = '2'); --8번쨰 자리에서 1번쨰만 자른 값 
                               
----------------------------------------------------------------------------------------
--4.다중행 다중열 서브쿼리
-- 서브쿼리 조회 결과 행 수 와 열 수가 여러개 일때
...
-- 해당 행이 서브쿼리의 조건을 충족하는 것을 확인하여 SELECT를 진행함
 ...
 * 따라서 각 직워느이 MANAGER_ID가 다른 직원의 EMP_ID와 일치하는 경우에
 * 해당 직원의 정보를 반환함
 *
 * */
 ...
 * */
 ...
 --부서별 입사일이 가장 빠른 사원의
 --사번,이름, 부서명(NULL이면 '소속없음')  NVL(DEPT_TITLE, '소속없음'), 직급명, 입사일 조회하고
 --입사일이 빠른순으로 정렬하라
 --단, 퇴사한 직원은 제외                   

 -- 메인쿼리 사번,이름, 부서명(NULL이면 '소속없음'), 직급명, 입사일 조회하고
 --입사일이 빠른순으로 정렬하라 단, 퇴사한 직원은 제외     
 ...
 *3. 전지연 입사일이랑 같음. 전지연이 D1부서의 가장 빠른 입사자 (RESULT SET 포함)
 * 
 * */
...
--(스칼라 + 상관쿼리)
SELECT EMP_NAME,JOB_CODE,SALARY,
       (SELECT CEIL (AVG(SALARY))  --CEIL : 올림처리
        FROM EMPLOYEE SUB
...
WHERE ROWNUM <= 3;  --ROWNUM은 WHERE절에서만 사용 할수 O / 확인필요할떄 SELECT절에서 사용해서 확인하기
...
--RANK() OVER : 동일한 순위 이후의 등수를 동일한 인원 수 만큼 건너뛰고 계산

 ...
 * [CACHE 바이트 크기 | NOCACHE] --캐시메모리 기본값은 20바이트 , 최소값은 2바이트
 * 
 * --시퀀스의 캐시메모리는 할당된 크기만큼 미리 다음 값들을 생성해 저장해둠
 * --> 시퀀스 호출 시 미리 저장되어진 값들을 가져와 변환하므로
 ...
 *                       단, 시퀀스 생성 후 NAEXTVAL 호출 없이 CURRVAL를 호출하면 오류 발생.
 *  
 * */
...
SELECT SEQ_TEST.CURRVAL FROM DUAL; --1 --NEXTVAL 해준 이후에 CURRVAL해주면 문제 없이 출력된다.
...
SELECT SEQ_TEST.CURRVAL FROM DUAL; --5  -- CURRVAL는 마지막 NEXTVAL보여주는거니까 5가 나온다!

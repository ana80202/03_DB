...
--기존에 서로 다른 테이블의 데이터를 조회 할 겨웅 아래와 같이 따로 조회함

--직원번호, 직원명, 부서코드, 부서명을 조회하고자 할때
SELECT EMP_ID,EMP_NAME,DEPT_CODE
FROM EMPLOYEE;

--부서명은 DEPARTEMT 테이블에서 조회 가능
...
--DEPARTMENT테이블, LOCATION_ID 테이블을 참조하여
--부서명, 지역명 조회
...
--컬럼수를 기준으로 JOIN 
--> 왼편에 작성된 테이블의 모든행의 결과에 포함되어야 한다
-- (JOIN이 안되는 행도 결과 포함)
...
-- 모든 행을 결과에 포함
--** 오라클 구문 FULL OUTER JOIN 을 사용 못함**
...
FROM EMPLOYEE
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND MAX_SAL);  --ORA-00918: 열의 정의가 애매합니다
...
FROM EMPLOYEE
JOIN EMPLOYEE ON(MANAGER_ID = EMP_ID); -- ORA-00918: 열의 정의가 애매합니다

SELECT E1.EMP_ID,E1.EMP_NAME,E1.MANAGER_ID ,E2.EMP_NAME
FROM EMPLOYEE E1
JOIN EMPLOYEE E2 ON(E1.MANAGER_ID = E2.EMP_ID);  --INNER조인된거랑 컬럼값이 일치하는 것만 -- 사수가 없는 사람들은 안나옴!
...
-->EMPLOYEE, LOCATION 은 같은 컬럼이 없어 XX
-->EMPLOYEE, DEPARTMENT  조인   +   DEPARTMENT,LOCATION 조인 이렇게 해서 연결해주기
...
AND LOCATION_ID = LOCAL_CODE; -- (EMPLOYEE + DEPARTMENT) + LOCATION조인 
...
--EMP_ID, EMP_NAME, JOB_CODE,DEPT_TITLE,LOCAL_NAME,SALARY
...
AND DEPT_CODE = DEPT_ID
AND LOCATION_ID = LOCAL_CODE
AND LOCAL_CODE = 'L1' AND LOCAL_CODE = 'L2';
...
FROM EMPLOYEE
NATURAL JOIN JOB
JOIN SAL_GRADE USING(SAL_LEVEL)  --같은 컬럼명 USING사용 가능
